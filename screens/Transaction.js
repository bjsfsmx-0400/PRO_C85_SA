...
export default class TransactionScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    if (!transactionType) {
      ...
      // Solo para usuarios de Android.
      // ToastAndroid.show("Se le envió el libro al alumno.", ToastAndroid.SHORT);
      Alert.alert("Se le envió el libro al alumno.");
    } else {
    ...
    if (studentRef.docs.length == 0) {
      ...
      studentRef.docs.map(doc => {
        if (doc.data().number_of_books_issued < 2) {
          ...
          isStudentEligible = false;
          Alert.alert("Ya se le enviaron 2 libros al alumno.");
          this.setState({
            bookId: "",
            studentId: ""
          });
  ...
  checkStudentEligibilityForBookReturn = async (bookId, studentId) => {
    ...
    transactionRef.docs.map(doc => {
      ...
      if (lastBookTransaction.student_id === studentId) {
        ...
        isStudentEligible = false;
        Alert.alert("El alumno no envió este libro.");
        this.setState({
          bookId: "",
          studentId: ""
        });
